...
my $path = '/usr/scratch/aniruddh/spin/new_results/dfly/multi_vc/ugal/'.$traffic_pattern;
...
for(my $ir=$start_inj_rate; $ir<$end_inj_rate; $ir = $ir + $step)
{
    ...
    $cmd = '/nethome/aramrakhyani3/spin/gem5/build/Garnet_standalone/gem5.debug -d '.$sim_path.' /nethome/aramrakhyani3/spin/gem5/configs/example/garnet_synth_traffic.py --network=garnet2.0 --num-cpus=1024 --num-dirs=1 --topology=dragonfly --dfly-group-size=8 --sim-cycles=20000 --injectionrate='.$ir.' --synthetic='.$traffic_pattern.' --enable-spin-scheme=1 --dd-thresh=128 --routing-algorithm=ugal --max-turn-capacity=50 --enable-variable-dd=0 --vcs-per-vnet=3 --enable-rotating-priority=1 --enable-dfly-dlock-avoidance=0 > '.$path.'/temp.txt';
