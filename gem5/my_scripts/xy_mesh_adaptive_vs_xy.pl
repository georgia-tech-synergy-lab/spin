...
my $path = '/usr/scratch/aniruddh/spin/adaptive_vs_xy_mesh/xy/'.$traffic_pattern;
...
for(my $ir=$start_inj_rate; $ir<$end_inj_rate; $ir = $ir + $step)
{
    ...
    $cmd = '/nethome/aramrakhyani3/spin/gem5/build/Garnet_standalone/gem5.debug -d '.$sim_path.' /nethome/aramrakhyani3/spin/gem5/configs/example/garnet_synth_traffic.py --network=garnet2.0 --num-cpus=64 --num-dirs=64 --topology=Mesh --mesh-rows=8 --sim-cycles=20000 --injectionrate='.$ir.' --synthetic='.$traffic_pattern.' --enable-spin-scheme=0 --dd-thresh=128 --routing-algorithm=xy --max-turn-capacity=30 --enable-variable-dd=1 --vcs-per-vnet=4 > /usr/scratch/temp.txt';
