...
#script to run Mesh topology with west_first (1 vcs-per-vnet) (64-cpus).


my $traffic_pattern = $ARGV[0];
chomp($traffic_pattern);

my $path = '/usr/scratch/aniruddh/spin/new_results/mesh/single_vc/west_first/'.$traffic_pattern;
...
for(my $ir=$start_inj_rate; $ir<$end_inj_rate; $ir = $ir + $step)
{
    ...
    $cmd = '/nethome/aramrakhyani3/spin/gem5/build/Garnet_standalone/gem5.debug -d '.$sim_path.' /nethome/aramrakhyani3/spin/gem5/configs/example/garnet_synth_traffic.py --network=garnet2.0 --num-cpus=64 --num-dirs=1 --topology=Mesh --mesh-rows=8 --sim-cycles=20000 --injectionrate='.$ir.' --synthetic='.$traffic_pattern.' --enable-spin-scheme=0 --dd-thresh=128 --routing-algorithm=west_first --max-turn-capacity=40 --enable-variable-dd=0 --vcs-per-vnet=1 --enable-rotating-priority=0 > '.$path.'/temp.txt';
