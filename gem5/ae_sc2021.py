...
for c in range(len(num_cores)):
	for b in range(len(bench)):
		...
		while(pkt_lat < 100.00 ):
			############ gem5 command-line ###########
			os.system("{0:s} -d {1:s}/{2:d}/{3:d}/{4:s}/{5:s}/vc-{6:d}/inj-{7:1.2f} configs/example/garnet_synth_traffic.py --topology=Mesh_XY --num-cpus={2:d} --num-dirs={2:d} --mesh-rows={8:d} --network=garnet2.0 --sim-cycles={9:d} --vcs-per-vnet={6:d} --inj-vnet=0 --injectionrate={7:1.2f} --synthetic={10:s} --enable-spin-scheme=1 --dd-thresh={3:d} --routing-algorithm=table --max-turn-capacity=40 --enable-variable-dd=0 --enable-rotating-priority=1".format(binary, out_dir, num_cores[c], thresh_, bench_caps[b], routing_algorithm[rout_], vc_, injection_rate, num_rows[c], cycles, bench[b]))

			# convert flot to string with required precision
			inj_rate="{:1.2f}".format(injection_rate)

			output_dir=("{0:s}/{1:d}/{2:d}/{3:s}/{4:s}/vc-{5:d}/inj-{6:1.2f}".format(out_dir, num_cores[c], thresh_, bench_caps[b], routing_algorithm[rout_], vc_, injection_rate))
